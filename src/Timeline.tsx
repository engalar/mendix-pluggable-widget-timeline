...
import  Timeline  from "@alifd/next/lib/timeline/mobile";
...
export default function (props: TimelineContainerProps) {
    const data = useMemo(() => {
        if (props.datasource && props.datasource.status === ValueStatus.Available) {
            return props.datasource.items!.map(d => ({ title: props.attrTitle.get(d).value!, content: props.attrContent.get(d).value! }));
        }
        return [];
    }, [props.datasource]);
